...
i.e.
        ...
        and {time_array} obatained from training data using 
...
"""
loads all interpolation data for the following modes from h5py data
...
def load_surrogate(EMRI):
    ...
    if EMRI == False:
        ...
        return 0
"""
takes the amplitude and phase of the waveform and computes the compose them together
...
"""
function to compute the interpolated waveform for a single mode
...
def slog_surrogate(q, h_eim_amp_spline, h_eim_ph_spline, eim_indicies_amp, eim_indicies_ph, B_amp, B_ph):
    ...
    return h_approx
"""
this function takes the interpolation indices, spline nodes, matrix B and computes the interpolated waveform for all modes
...
"""
calls the load surrogate function
once called the data is loaded
